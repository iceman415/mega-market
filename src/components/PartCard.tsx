...
export function PartCard({ part, onClick }: PartCardProps) {
  ...
  const nextImage = useCallback((e?: React.MouseEvent) => {
    e?.stopPropagation();
    dir.current = 1;
    setCurrentImage((prev) => (prev + 1) % part.images.length);
  }, [part.images.length]);

  const prevImage = useCallback((e?: React.MouseEvent) => {
    e?.stopPropagation();
    dir.current = -1;
    setCurrentImage((prev) => (prev - 1 + part.images.length) % part.images.length);
  }, [part.images.length]);
  ...
  return (
    ...
    >
      ...
      <div className="p-4">
        <h3 className="font-oswald text-xl text-gray-900 mb-1">{part.name}</h3>
