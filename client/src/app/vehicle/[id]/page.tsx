...
import { VehicleDetailSkeleton } from "@/components/VehicleSkeleton";
...
export default function VehicleDetailPage({
  ...
  const images =
    vehicle && vehicle.images.length > 0
      ? vehicle.images
      : ["/placeholder.svg"];
  ...
  const handleDragEnd = useCallback(
    (_: unknown, info: { offset: { x: number }; velocity: { x: number } }) => {
      ...
      if (v > threshold) goPrev();
      else if (v < -threshold) goNext();
    },
    [goNext, goPrev],
  );
  ...
  if (!vehicle) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 gap-4">
        <p className="font-oswald text-2xl text-gray-900">Vehicle not found</p>
        <Link
          href="/"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          >
            ...
            {vehicle.youtubeUrl && (
              <div className="mt-6">
                <YouTubeEmbed url={vehicle.youtubeUrl} />
              </div>
            )}
          </motion.div>
          ...
          >
            ...
            {!vehicle.sold && (
              <motion.a
                href={`/contact?item=vehicle&id=${vehicle.id}`}
                whileHover={{ scale: 1.03 }}
                whileTap={{ scale: 0.97 }}
