...
import { useClothingItem } from "@/hooks";
...
import { ClothingDetailSkeleton } from "@/components/ClothingSkeleton";
...
export default function ClothingDetailPage({
  ...
  const images =
    clothing && clothing.images.length > 0 ? clothing.images : ["/placeholder.svg"];
  ...
  if (isLoading) {
    return <ClothingDetailSkeleton />;
  }

  if (!clothing) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 gap-4">
        <p className="font-oswald text-2xl text-gray-900">Clothing not found</p>
        <Link
          href="/"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          >
            ...
            {clothing.youtubeUrl && (
              <div className="mt-6">
                <YouTubeEmbed url={clothing.youtubeUrl} />
              </div>
            )}
          </motion.div>
          ...
          >
            ...
            {!clothing.sold && (
              <motion.a
                href={`/contact?item=clothing&id=${clothing.id}`}
                whileHover={{ scale: 1.03 }}
                whileTap={{ scale: 0.97 }}
