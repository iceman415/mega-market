...
import { Plus, Edit3, Trash2, ArrowLeft, Search, Home, Settings } from "lucide-react";
import { useClothing, useDeleteClothing } from "@/hooks";
...
export default function AdminClothingPage() {
  ...
  const { data: clothing, isLoading } = useClothing();
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success("Clothing deleted");
    } catch {
      ...
      <header className="bg-white shadow-sm border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-wrap items-center justify-between gap-y-2">
          <div className="flex items-center gap-3">
            ...
            <h1 className="font-oswald text-xl text-gray-900">Clothing</h1>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <Link href="/admin/clothing/new">
              <Button className="bg-mega-blue hover:bg-mega-blue-dark text-white flex items-center gap-2 rounded-xl">
                ...
                <span className="hidden sm:inline">Add Clothing</span>
              </Button>
            </Link>
          </div>
        </div>
              ...
              >
                <Dialog>
                  <DialogTrigger nativeButton={false} render={<span />}>
                    <button
                      type="button"
                      onClick={() => setPreviewUrl(item.images[0] || null)}
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(o) => !o && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. The item and its images will be permanently removed.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
